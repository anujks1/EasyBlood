...
public class NsMenuAdapter extends ArrayAdapter<NsItemModel> {
    ...
    public NsMenuAdapter(Context context, NsItemModel[] resource) {
        super(context, com.app.easyblood.R.layout.notification_custom_list_item, resource);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int layout = com.app.easyblood.R.layout.notification_custom_list_item;

        view = LayoutInflater.from(getContext()).inflate(layout, null);

        TextView text1 = (TextView) view.findViewById(com.app.easyblood.R.id.ns_title);
        ImageView image1 = (ImageView) view.findViewById(com.app.easyblood.R.id.ns_icon);
        TextView textcounter1 = (TextView) view.findViewById(com.app.easyblood.R.id.ns_counter);
        view.setTag(new ViewHolder(text1, image1,textcounter1));

        if(item.title.equals("Home")){
            image1.setImageResource(com.app.easyblood.R.drawable.home);
        }
        if(item.title.equals("Chat")){
            image1.setImageResource(com.app.easyblood.R.drawable.chat);
        }
        if(item.title.equals("About")){
            image1.setImageResource(com.app.easyblood.R.drawable.info);
        }
        if(item.title.equals("Questions")){
            image1.setImageResource(com.app.easyblood.R.drawable.question);
        }
        if(item.title.equals("Requests")){
            image1.setImageResource(com.app.easyblood.R.drawable.requests);
        }
        if(item.title.equals("My Questions")){
            image1.setImageResource(com.app.easyblood.R.drawable.my_questions);
        }
        if(item.title.equals("My Requests")){
            image1.setImageResource(com.app.easyblood.R.drawable.my_requests);
        }
